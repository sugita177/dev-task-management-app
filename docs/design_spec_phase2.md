...
### 🔹 Step 2: 全域API認証保護とユーザーID連携 (バックエンド完了)
* [x] 全コントローラー（`TaskController`, `ProjectController`, `UserController`）への `JwtAuthGuard` 適用。(完了)
* [x] バックエンド側での `req.user.userId` を用いた作成者・更新者・コメント投稿者の強制バインディング。(完了)
* [x] Access Token 寿命の 15分 (`15m`) 化と `POST /api/auth/refresh` の追加。(完了)

### 🔹 Step 3: データモデリング、組織マスタ完全API化 & フロントエンド自動トークン更新
* `organizations` テーブルの作成、`users` の関連付け。
* ヘッダー組織名の動的化と、カテゴリマスター（`GET /api/categories`）の構築による完全なモック脱却。
* **[新規追加] フロントエンド Axios 401 インターセプター (15分トークン自動更新):**
  トークン切れ（401）検知時に、裏で自動的に `/api/auth/refresh` を呼んでアクセス・トークンを再取得し、ユーザーにログイン画面を出さずシームレスにリトライする基盤を本ステップのフロントエンド共通通信整備にて実装。
...
### 🔹 Step 6: 認可 (RBAC 権限管理) & ビジネス・プロフェッショナル向け機能の追加
* **[新規追加] 認可 (Authorization / RolesGuard) の実装:**
  NestJS に `@Roles('ADMINISTRATOR')` などの `RolesGuard` を導入し、「誰がログインしているか（認証）」だけでなく「そのロールに実行権限があるか（認可）」をチェック。管理者専用画面（`/admin/users`）やプロジェクト削除などのアクセス制御を本ステップにて完成させる。
