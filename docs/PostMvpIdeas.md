...
### ④ 開発者専用の「フォーカスモード（マイ・タスク）」とタイマー永続化
* **概要:** 
...
* **24時間経過タイマーの確認ダイアログ（ユーザー判断委ね設計）:**
  タイマー開始時刻から24時間が経過した場合でも自動削除・自動記録は行わず、`localStorage` 上にそのままタイマー状態を保持・復元します。その上でユーザーに「24時間以上経過したタイマーが見つかりました。削除しますか？」と確認ダイアログを表示し、削除するかそのまま継続・手動保存を行うかは完全にユーザーの操作に委ねる柔軟な設計とします。
* **バックエンド (Redis / DB) での活性タイマー永続化 (マルチデバイス同期・チームリアルタイム可視化):**
  将来拡張として、タイマー状態をフロントエンド (`localStorage`) だけでなくバックエンド（Redis または `active_time_trackers` テーブル）へ保存するAPIを構築。PCで開始したタイマーをスマホで停止・継続したり、EMダッシュボード上で「今誰がどのタスクを計測中か」をリアルタイム可視化できるようにします。
* **日跨ぎタイムトラッキングの自動按分分割（00:00 日付別分割）:**
  タイマー計測中に深夜 00:00 を跨いだ場合、現在は保存ボタン押下日の実績として記録されますが、将来的に日付ごとに自動分割（例: 23:00〜01:00 の場合、前日1h＋当日1h）して2つのワークログを生成する高度な日別集計機能。
* **誤記録した実績工数（`work_logs`）の修正・削除機能:**
  誤って記録してしまった作業時間やメモを後から削除・修正するための API (`DELETE /tasks/:id/work-logs/:workLogId`) および詳細モーダル「実績工数」タブへのゴミ箱/編集アイコンの追加。
* **組織共通の休日・休暇カレンダー設定機能（年末年始・夏季休暇・祝日カスタマイズ）:**
  管理者（`ADMINISTRATOR`）が組織独自の特別休暇（年末年始休暇、夏季特別休暇、創業記念日等）や祝日カレンダーを柔軟に登録・設定できるマスタ管理機能（`organization_holidays` テーブル / API）。アサイン状況画面の「今月の実営業日数」やキャパシティ計算において設定された休日を自動的に除外控除し、組織の実態に完全に即した精緻な過負荷分析を可能にします。
* **ガントチャート進捗率（%）の精緻化（実績工数比率 / サブタスク完了率による動的自動算出）:**
  現在はステータス (`DONE`=100%, `IN_REVIEW`=75%, `IN_PROGRESS`=50%, `BACKLOG`=0%) による段階的表示ですが、将来的に `SUM(work_logs.hours) / estimatedHours`（実績時間の消化割合）やサブタスクの完了割合と連動させて、1% 刻みで自動更新されるリアルタイム進捗率算出機能。
...
### ④ 移動先タスク未検出時（削除・データ破損・非表示等）のフォールバック安定化
* **内容:** かんばんボードやガントチャートで先行・後続タスクへスクロール移動を試みた際、データ破損や削除、特殊なフィルター等により移動先DOM要素が見つからない場合、サイレントに失敗させず「対象タスクが見つかりませんでした」のアラート通知を表示、または直接詳細モーダルを開くフォールバック制御を導入してユーザー体験を安定化させる。
